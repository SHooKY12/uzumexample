...
class MainPageFragment : Fragment(R.layout.activity_main_page) {
    ...
    private var listForTablayoutText = listOf("Распродажа", "Популярные", "Новинки")
    ...
    override fun onViewCreated(view: View, savdInstanceState: Bundle?) {
        // Добавления картинов в список для баннера
        initArrayImgs()
        // Инициализация RecyclerView для баннеров
        recyclerView = view.findViewById(R.id.rvBanners)
        adapter.setImages(imgList)
        bindRecycler()
        // Инициализация SwipeRefreshLayout и установка Listener
        swiperRefresher = view.findViewById(R.id.swiperRefresher)
        instalListenerOnSwipeRefresh()
        // Инициализация ViewPager2 и TabLayout
        viewPage2 = view.findViewById(R.id.placeHolder)
        viewPage2.isUserInputEnabled = false
        tableLayoit = view.findViewById(R.id.tableLayoutInMainPage)
        initTablayout()
    }
    ...
    private fun initTablayout() {
        ...
        val adapterTabl = TableLayoutAdapter(requireActivity(), tableItems)
